...
# ── NumPy 2.x Safety Check (non-fatal — dashboard still loads) ────────
_NUMPY_2X = np.__version__.startswith("2.")
if _NUMPY_2X:
    st.warning(
        f"⚠️ **NumPy {np.__version__} detected.** "
        "Live detection may be limited on this environment. "
        "Demo Mode, Analytics, Map, and Reports work normally. "
        "To fix: ensure `numpy<2.0.0` in `requirements.txt` and reboot the app."
    )
...
# ── Damage Classes & Constants ─────────────────────────────────────────
try:
    from road_quality_monitor.constants import (
        CLASS_NAMES, SEVERITY_BGR, SEVERITY_RGB, ROAD_DAMAGE_CLASSES, SEVERITY_BADGES, DARK_THEME
    )
except ImportError:
    try:
        ...
        SEVERITY_RGB = {"HIGH": (220, 0, 0), "MEDIUM": (255, 165, 0), "LOW": (50, 200, 50)}
        ROAD_DAMAGE_CLASSES = {"pothole", "crack", "wear"}
        SEVERITY_BADGES = {"HIGH": '🔴 HIGH', "MEDIUM": '🟡 MEDIUM', "LOW": '🟢 LOW'}
        DARK_THEME = {"accent_blue": "#58a6ff"}
...
.status-online  { color: var(--accent-green); border-color: rgba(63, 185, 80, 0.4); }
.status-offline { color: var(--accent-red); border-color: rgba(248, 81, 73, 0.4); }
.status-warning { color: var(--accent-orange); border-color: rgba(210, 153, 34, 0.4); }

/* Animation for pulsating status */
@keyframes pulse {
    0%   { opacity: 1; }
    50%  { opacity: 0.5; }
    100% { opacity: 1; }
...
with col_v:
    ...
    if st.session_state["is_detecting"] and "🎥" in mode:
        ...
        # IDLE placeholder — branded card instead of a black image
        video_slot.markdown("""
        <div style="
            height: 360px;
            background: #161b22;
        ...
        ">
            ...
            <div style="font-size: 1.15rem; font-weight: 700; color: #58a6ff; letter-spacing: 0.05em;">
                SYSTEM IDLE
            </div>
            <div style="font-size: 0.85rem; margin-top: 8px; color: #8b949e;">
                Press <strong style="color:#e6edf3;">▶ Start Detection</strong> to begin live analysis
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    try:
        ...
        if not weights.exists():
            ...
            if video_source == "webrtc":
                ...
                if sys.platform == "win32" and isinstance(cap_src, int):
                ...
                if not cap.isOpened():
                    ...
                    while st.session_state["is_detecting"]:
                        ...
                        # ── Hardware Diagnostics: Check for pitch-black frames ──
                        if np.mean(frame) < 2.0:  # Frame is almost entirely black
                            if not black_frame_warning_shown:
                                st.warning("⚠️ **Camera is capturing a black screen.** Check if your laptop privacy shutter is closed, or if another app is using the camera.")
                                black_frame_warning_shown = True
                            
                            # Draw a helpful message on the black frame itself
                            cv2.putText(frame, "NO SIGNAL / BLACK FRAME", (130, 240), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
                            cv2.putText(frame, "Check privacy shutter or select different Camera Index", (60, 280), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
